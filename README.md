...
A estrutura do repositório está pronta. **Ainda não há código de back nem de front**, e nada foi
commitado no Git. As pastas `docs/marca/` e `docs/styleguide/` estão vazias, esperando as escolhas
de marca e de estilo.

```
.
├── docs/            o que não é código
│   ├── CARTILHA.md  a cartilha sorteada, sem alteração
│   ├── BRIEFING.md  a escrever
...
│   └── rotas/  servicos/  repositorios/  esquemas/
...
Com o `main.py` escrito, o esperado é:

- o terminal mostrar `http://localhost:8000`;
- abrir `http://localhost:8000/docs` e ver a lista de rotas;
- `http://localhost:8000` responde com `{"mensagem": ...}`, se a rota de raiz existir.
